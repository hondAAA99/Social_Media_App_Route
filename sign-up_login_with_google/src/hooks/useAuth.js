...
export const useAuth = () => {
    const { user, isAuthenticated, isLoading, error, setUser, setAccessToken: setAuthToken, setIsLoading, setError, login, logout: logoutStore, updateUser, } = useAuthStore();
    ...
    const confirmSignUp = useCallback(async (token) => {
        setIsLoading(true);
        setError(null);
        try {
            ...
            const message = err.response?.data?.message || "Email confirmation failed";
            setError(message);
            return { success: false, error: message };
        }
        finally {
    ...
    const confirmLogin = useCallback(async (code) => {
        setIsLoading(true);
        setError(null);
        try {
